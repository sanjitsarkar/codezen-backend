...
const compileCodeController = async (req, res) => {
  ...
  if ((await exists(rpath)) === false && _code) await fs.mkdir(rpath);
  if (_code) {
    try {
...
${code}`;
        ...
        var codes = code.replace(`main() {`, _code);
      ...
      console.log(e);
      res.status(501).send({ errors: errors.toString() });
    }
  }
};
